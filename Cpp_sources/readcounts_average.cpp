...
int main(int argc, char* argv[]) {
  if (argc != 3) {
	std::cout << "readcounts_average <ranges> <counts>\t Reads ranges [from; to) or lengths for each identifier from <ranges> in tab-separated values file format; and\n";
	std::cout << "                                    \t computes an average read count within the region from <counts> file in tab-separated values file format.\n\n";
  ...
  // <id, coef> Average read count within a region for each identifier
  std::map<std::string, double> coefs;
  {
	...
	for (std::string line; std::getline(counts_file, line);) {
	  ...
	  if (ranges_it->second.first <= pos && pos < ranges_it->second.second) {
		coefs[id] += std::stod(line.substr(tab_second + 1)) / (ranges_it->second.second - ranges_it->second.first);
	  }
	}
  }
